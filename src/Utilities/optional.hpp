...
namespace xppr {
    ...
    public:
        ...
        /*implicit*/constexpr Optional(Err err = ErrHasNoValue) noexcept : m_err(err) { assert(err != NoErr); }
        ...
        /*implicit*/constexpr Optional(const T& t) noexcept : m_err(NoErr) { new(m_data) T(t); }
        ...
        /*implicit*/constexpr Optional(T&& t) noexcept : m_err(NoErr) { new(m_data) T(std::move(t)); }
        ...
        // Copy constructor
        constexpr Optional(const Optional& oth) noexcept : m_err(oth.m_err) { if(oth){ new(m_data) T(*oth);} }
        ...
        //Emplace constructor 
        template<class... Args >
        constexpr Optional(Args&&... args ) { emplace(std::forward<Args>(args)...); }
        ...
        constexpr       T& operator*()        { requireValue(); return *reinterpret_cast<      T*>(m_data); }
        constexpr const T& operator*()  const { requireValue(); return *reinterpret_cast<const T*>(m_data); }
        ...
        constexpr std::strong_ordering operator<=>(Err err) const noexcept { return m_err <=> err; }
